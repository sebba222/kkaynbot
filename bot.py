...
def setup_sheets():
    ...
    wp=sp.add_worksheet("Por Cuenta",rows=500,cols=18)
...
def update_por_cuenta():
    """3 bloques verticales, uno por banco. Cada bloque:
    - Fila banco: header azul oscuro, merge completo
    - Fila sub: PESOS(UYU) | sep | DÓLARES(USD) | sep | TOTALES
    - Fila col-headers
    - Filas de datos en paralelo UYU izq / USD der
    - Fila separadora
    Cols: 1-6=UYU, 7=sep, 8-13=USD, 14=sep, 15-18=totales
    """
    ctx=get_ctx()
    if not ctx: return
    sp=ss(); wp=sp.worksheet("Por Cuenta"); wpc=wp._properties['sheetId']
    s=ctx["saldos"]; rate=ctx["rate"]; movs=ctx["movs"]
    H=['FECHA','DESCRIPCIÓN','CATEGORÍA','INGRESO','EGRESO','SALDO']
    HT=['SAL UYU','SAL USD','TODO UYU','TODO USD']
    wp.batch_clear(["A2:R500"])
    rqs=[]; bv=[]; cur=2

    for bname,cuyu,cusd in BANCOS:
        muyu=[r for r in movs if r[3]==cuyu]
        musd=[r for r in movs if r[3]==cusd]
        n=max(len(muyu),len(musd))
        suyu=s.get(cuyu,0); susd=s.get(cusd,0)
        tuyu=suyu+susd*rate; tusd=suyu/rate+susd if rate else susd

        # Fila header banco
        rqs+=[fr(wpc,cur,1,cur,18,bold=True,bg=AZ_OSC,fg=T_BLA,sz=12,al="CENTER"),
              mg(wpc,cur,1,cur,18),rh(wpc,cur,34)]
        bv.append({"range":f"A{cur}","values":[[f"🏦  {bname}"]]})
        cur+=1

        # Fila sub-headers
        rqs+=[fr(wpc,cur,1,cur,6,bold=True,bg=TURQ,fg=T_BLA,sz=10,al="CENTER"),mg(wpc,cur,1,cur,6),
              ...
              fr(wpc,cur,15,cur,18,bold=True,bg=AZ_MED,fg=T_BLA,sz=10,al="CENTER"),mg(wpc,cur,15,cur,18),
              rh(wpc,cur,26)]
        ...
        # Fila col-headers
        rqs+=[fr(wpc,cur,1,cur,6,bold=True,bg=GR_OSC,fg=T_BLA,al="CENTER"),
              fr(wpc,cur,7,cur,7,bg=BLANCO),
              fr(wpc,cur,8,cur,13,bold=True,bg=GR_OSC,fg=T_BLA,al="CENTER"),
              fr(wpc,cur,14,cur,14,bg=BLANCO),
              fr(wpc,cur,15,cur,18,bold=True,bg=GR_OSC,fg=T_BLA,al="CENTER"),
              rh(wpc,cur,24)]
        ...
        # Totales en primera fila de datos
        bv.append({"range":f"O{cur}","values":[[f"$ {suyu:,.0f}",f"U$S {susd:,.2f}",f"$ {tuyu:,.0f}",f"U$S {tusd:,.2f}"]]})
        rqs.append(fr(wpc,cur,15,cur,18,bold=True,bg=AZ_CLA,fg=AZ_OSC,al="CENTER"))
        ...
        # Separador
        rqs+=[fr(wpc,cur,1,cur,18,bg=BLANCO),rh(wpc,cur,14)]
        cur+=1
